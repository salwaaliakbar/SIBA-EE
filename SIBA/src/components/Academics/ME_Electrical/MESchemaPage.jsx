      ...
      {semesters.length > 0 && (
        <section className="border-y border-slate-200 bg-white">
          <div className="mx-auto max-w-[1440px] px-5 py-14 sm:px-6 lg:px-10 lg:py-20">
            ...
            <div className="mt-10 space-y-8">{semesters.map((semester, index) => <Reveal key={semester.title} delay={(index % 2) * 0.05} className="overflow-hidden border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-[#071f47] px-5 py-4 text-white"><h3 className="font-serif text-2xl font-bold">{semester.title}</h3><span className="font-mono text-xs uppercase tracking-wider text-amber-300">{semester.total}</span></div><CourseTable courses={semester.courses} /></Reveal>)}</div>
